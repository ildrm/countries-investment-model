...
# -*- coding: utf-8 -*-
"""
United States Investment Macro Model
==================================
...
COUNTRY_CODE = "US"
CURRENCY = "USD"
CENTRAL_BANK = "Board of Governors of the Federal Reserve System"
...
INFLATION_REFERENCE_NOTE = "2% is the Fed's longer-run PCE inflation goal; CPI is used here as an accessible live proxy."
GROWTH_SCALE = 3.0
...
SEED_SNAPSHOT = {'inflation_yoy': {'value': 3.4, 'unit': 'percent', 'period': '2026-07', 'release_date': '2026-08-12', 'frequency': 'monthly', 'source': 'Federal Reserve Bank of St. Louis FRED / CPIAUCNS (source: BLS)', 'source_url': 'https://fred.stlouisfed.org/series/CPIAUCNS', 'base_confidence': 0.92}, 'gdp_growth': {'value': 2.1, 'unit': 'percent_annualized_qoq', 'period': '2026-Q1', 'release_date': '2026-06-25', 'frequency': 'quarterly', 'source': 'Federal Reserve Board - Economy at a Glance: GDP', 'source_url': 'https://www.federalreserve.gov/economy-at-a-glance-gross-domestic-product.htm', 'base_confidence': 0.9}, 'policy_rate': {'value': 3.63, 'unit': 'percent', 'period': '2026-08-19', 'release_date': '2026-08-20', 'frequency': 'daily', 'source': 'Federal Reserve Board H.15 - effective federal funds rate', 'source_url': 'https://www.federalreserve.gov/releases/h15/', 'base_confidence': 0.98}}
...
def _fred_series(client: HttpClient, series_id: str) -> list[tuple[str, float]]:
...
def _yoy(rows: list[tuple[str, float]], periods: int) -> Optional[tuple[str, float]]:
    if len(rows) <= periods:
        return None
    latest_date, latest = rows[-1]
    _, old = rows[-1 - periods]
    if old == 0:
        return None
    return latest_date, (latest / old - 1.0) * 100.0
...
def fetch_live(client: HttpClient) -> tuple[dict[str, Observation], list[str]]:
    ...
    try:
        cpi = _fred_series(client, "CPIAUCNS")
        value = _yoy(cpi, 12)
        if value:
            period, yoy = value
            out["inflation_yoy"] = obs(
                "inflation_yoy", yoy, period=period, frequency="monthly",
                source="FRED, Federal Reserve Bank of St. Louis / CPIAUCNS (BLS source)",
                source_url="https://fred.stlouisfed.org/series/CPIAUCNS",
                base_confidence=0.94,
            )
    except Exception as exc:
        warnings.append(f"FRED CPI retrieval failed: {exc}")

    try:
        core = _fred_series(client, "CPILFESL")
        value = _yoy(core, 12)
        if value:
            period, yoy = value
            out["core_inflation"] = obs(
                "core_inflation", yoy, period=period, frequency="monthly",
                source="FRED, Federal Reserve Bank of St. Louis / CPILFESL (BLS source)",
                source_url="https://fred.stlouisfed.org/series/CPILFESL",
                base_confidence=0.94,
            )
    except Exception as exc:
        warnings.append(f"FRED core-CPI retrieval failed: {exc}")

    try:
        m2 = _fred_series(client, "M2SL")
        value = _yoy(m2, 12)
        if value:
            period, yoy = value
            out["money_growth"] = obs(
                "money_growth", yoy, period=period, frequency="monthly",
                source="FRED, Federal Reserve Bank of St. Louis / M2SL (Federal Reserve source)",
                source_url="https://fred.stlouisfed.org/series/M2SL",
                base_confidence=0.96,
            )
    except Exception as exc:
        warnings.append(f"FRED M2 retrieval failed: {exc}")

    try:
        dff = _fred_series(client, "DFF")
        if dff:
            period, rate = dff[-1]
            out["policy_rate"] = obs(
                "policy_rate", rate, period=period, frequency="daily",
                source="FRED / effective federal funds rate",
                source_url="https://fred.stlouisfed.org/series/DFF",
                base_confidence=0.98,
                note="Effective rate used as the live policy-rate proxy.",
            )
    except Exception as exc:
        warnings.append(f"FRED effective-fed-funds retrieval failed: {exc}")

    try:
        gdp = _fred_series(client, "GDPC1")
        if len(gdp) >= 2:
            period, latest = gdp[-1]
            prev = gdp[-2][1]
            growth = ((latest / prev) ** 4 - 1.0) * 100.0 if prev else 0.0
            out["gdp_growth"] = obs(
                "gdp_growth", growth, period=period, frequency="quarterly",
                source="FRED / real GDP GDPC1 (BEA source)",
                source_url="https://fred.stlouisfed.org/series/GDPC1",
                base_confidence=0.92,
                unit="percent_annualized_qoq",
                note="Annualized quarter-over-quarter real GDP growth.",
            )
    except Exception as exc:
        warnings.append(f"FRED GDP retrieval failed: {exc}")
...
def apply_valuation(
    ...
    if "equity_forward_pe" in market:
        ...
        fair = float(market.get("equity_fair_pe", 18.0))
    ...
    if "housing_price_to_annual_rent" in market:
        ...
        fair = float(market.get("housing_fair_ratio", 20.0))
